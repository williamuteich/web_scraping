...
interface NavBuscarProps {
  buscar: string;
  onChange: (termo: string) => void; // Adicione a propriedade onChange
}
    ...
    <div className="p-2 pl-6 pr-6 flex items-center justify-center border border-solid border-[#00000029] rounded-sm">
      <div className='flex gap-20 justify-between items-center w-full'>
        ...
        <div className='flex gap-4 justify-between'>
          ...
          <button onClick={handleAtualizarClick} className="bg-[var(--corPrincipal)] text-white text-sm pl-6 pr-6 rounded-lg border border-solid border-black hover:bg-white hover:text-[var(--corPrincipal)] transition duration-300">
            Atualizar
          </button>
        </div>
      </div>
